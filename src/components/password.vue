<template>
	<div class="box">
        ...
        <div class="form">
            ...
            <div class="active"><label>确认密码</label><input type="password" placeholder="请输入确认密码" v-model="pass"/></div>
            <div class="active"><label>图形验证码</label><input style="width: 30%;" type="text" placeholder="请输图形验证码" v-model="captchaImage"/><img class="Img" id="Img" @click="refreshCaptcha()" src='http://120.24.160.251:8081/application/imgCode?+ Math.random()'/></div>
            <div>
                <label>短信验证码</label>
	  ...
	  name: 'password',
		...
		methods: {
			refergo () {
                ...
                formData.append("captchaImage", this.captchaImage); 
                formData.append("code", this.code); 
                axios({
                    url: '/api/login/doResetPwd',
                    method: 'post',
                    data: formData,
                }).then(respanse=>{
            ...
            refreshCaptcha() {
                document.getElementById("Img").src = 'http://120.24.160.251:8081/application/imgCode?' + Math.random();
            }
        },
        mounted () {
